...
要完成一个大工程，就要先把它分解成多个小工程，再分解成更小的工程，直到力所能及（能完成）为止，然后逐个去完成它。
...
<table border="1" align="center">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>from package import subpackage</code>、<code>from package.subpackage.module import my_function1,my_function2</code></td>
    </tr>   
    <tr>
...
- **函数 vs 方法**：
  ...
  - **方法（method）**：绑定到一个类的实例​对象，方法必须通过 **​对象（object）** 调用。例如 `obj.method()`(`obj`是一个类的实例对象，`method`是该对象的方法)。方法是与类紧密相关的功能，它们操作的是该类的实例（对象）的数据（即属性）。

    函数类似于工具箱里的工具，拿来就用，不依赖其他东西，比如一把螺丝刀。

    方法类似于物品（对象）自带的功能，比如手机上的拍照按钮，你必须先有手机（对象），才能按拍照（方法）。
