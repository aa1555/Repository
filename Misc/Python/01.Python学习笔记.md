...
- 计算机储存的所有数据都是以二进制的形式储存的。二进制数据就是'万变不离其宗'里的'宗'。
  ...
  计算机里不同格式的数据，例如文字、图片、音频、视频，都是以不同的编码方案或编码标准经过一系列的转换和编码，最后转换成二进制的形式储存到电脑硬盘里的。而要打开这些文件，就要用对应格式的解码器进行解码。解码器的作用就是把二进制数据转换成人类能够识别的信息，例如文字、图片、音频、视频。所谓数字化，就是指将各种形式的信息（如文本、图像、声音等）转换为计算机可以处理和存储的数字形式的过程。
