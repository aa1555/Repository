...
money = 1000  # 初始资金1000元


while money > 0:    # 控制循环，只要money大于0，就可以一直玩下去。每一局一次循环，每一局都要判断一次，每一局分出胜负都从这开始，满足条件就继续，不满足条件就直接到底部print破产。
    print(f'你的总资产为: {money}元')  # f'{}'用来嵌入，print默认后面有一个\n换行符
    go_on = False  # 控制最下面那个while的循环
    while True:  # 下面如果不加一个break，它将一直循环，while true是一个死循环。
        debt = int(input('请下注: '))  # 定义一个下注的变量debt，input()用于从用户那里获取输入,实现与用户交互
